...
(function(){
  ...
  async function loadStatic(){if(embedded.length)return clone(embedded);const prefix=location.pathname.includes('/admin/')?'../data/':'data/';const out=[];for(let i=0;i<9;i++){try{const r=await fetch(prefix+'cat-'+i+'.json');if(r.ok){const c=await r.json();if(c)out.push(c)}}catch(e){}}return out}
  async function load(){if(!configured()){const local=localLoad();if(local.length)return local;return loadStatic()}try{const x=await rest('/catalog');if(x?.categories)return x.categories;if(Array.isArray(x))return x}catch(e){}return localLoad().length?localLoad():loadStatic()}
  ...
  window.PanoramaCatalog={initial:embedded,load,loadStatic,save,reset,clone,remoteConfigured:configured,subscribeCatalog,saveOrder,subscribeOrders,updateOrderStatus,getSettings,saveSettings,CATALOG_KEY,SETTINGS_KEY};
})();
